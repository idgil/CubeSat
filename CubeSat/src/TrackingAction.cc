...
TrackingAction::TrackingAction(DetectorConstruction* detector, EventAction* event)
    :fDetector(detector),eventAct(event)
{
    fDetector = detector;
    fTargetRegion = 0;
}
...
void TrackingAction::PreUserTrackingAction(const G4Track* track)
{
    ...
    if(particleDefinition == G4Electron::Definition() || particleDefinition == G4Gamma::Definition()
            || particleDefinition == G4OpticalPhoton::Definition() )
    {
 ...
 //   eventAct->AddChargeDist(1,1,-1.,-1.,-1.);
    }
